...
def init_db():
    ...
    c.executescript('''
        ...
        CREATE TABLE IF NOT EXISTS library_edits (
            article_id TEXT PRIMARY KEY,
            title TEXT,
            content TEXT,
    ...
    if c.fetchone()[0] == 0:
        seed = [
            ...
            ('Клей-момент универсальный', 'decor', 'g', 0.9, {}, 0),
            ...
            ('Силикон для форм (платиновый)', 'other', 'g', 3.5, {}, 0),
            ('Силикон для форм (оловянный)', 'other', 'g', 2.0, {}, 0),
            ('Воск разделительный', 'other', 'ml', 5.0, {}, 0),
            ('Спрей-разделитель', 'other', 'ml', 3.5, {}, 0),
        ]
        for s in seed:
            c.execute(
                "INSERT INTO materials(name, category, unit, price, meta, favorite) "
                "VALUES(?,?,?,?,?,?)",
                (s[0], s[1], s[2], s[3], json.dumps(s[4]), s[5]))

    # Сидирование шаблонов
    c.execute("SELECT COUNT(*) FROM templates")
    if c.fetchone()[0] == 0:
        seed_templates = [
            ('Подсвечник классический', 'gypsum',
             'Простая форма, 250 г гипса, 30 мин работы',
             json.dumps({'gypsum_kg': 0.25, 'labor_hours': 0.5, 'waste_pct': 5})),
            ('Панно декоративное (20×20)', 'gypsum',
             'С армированием, 1.2 кг гипса, 2 ч работы',
             json.dumps({'gypsum_kg': 1.2, 'labor_hours': 2, 'waste_pct': 8})),
            ('Бюст малый', 'gypsum',
             'Скульптурный гипс, 800 г, 3 ч работы',
             json.dumps({'gypsum_kg': 0.8, 'labor_hours': 3, 'waste_pct': 5})),
            ('Кулон-капля', 'resin',
             'Эпоксидка A 20 г, B 10 г, 30 мин работы',
             json.dumps({'resin_a_g': 20, 'resin_b_g': 10, 'ab_ratio': 2, 'labor_hours': 0.5})),
            ('Столешница круглая', 'resin',
             'Эпоксидка A 400 г, B 200 г, 4 ч работы',
             json.dumps({'resin_a_g': 400, 'resin_b_g': 200, 'ab_ratio': 2, 'labor_hours': 4})),
            ('Миниатюра 28мм', 'sla',
             'UV-смола 3 мл, 1.5 ч печати',
             json.dumps({'sub': 'sla', 'sla_ml': 3, 'print_hours': 1.5, 'printer_power': 60})),
            ('Функциональная деталь', 'fdm',
             'PLA 40 г, 3 ч печати',
             json.dumps({'sub': 'fdm', 'fdm_g': 40, 'print_hours': 3, 'printer_power': 150})),
        ]
        for t in seed_templates:
            c.execute(
                "INSERT INTO templates(name, type, description, data) VALUES(?,?,?,?)", t)
...
def load_library():
    ...
    conn = get_conn()
    edits = {r['article_id']: dict(r) for r in
             conn.execute("SELECT * FROM library_edits").fetchall()}
    bookmarks = {r['article_id'] for r in
                 conn.execute("SELECT article_id FROM bookmarks").fetchall()}
    conn.close()
    ...
    for cat in lib.get('categories', []):
        for art in cat.get('articles', []):
            ...
            if not aid:
                continue
            if aid in edits:
                if edits[aid].get('title'):
                    art['title'] = edits[aid]['title']
                if edits[aid].get('content'):
                    art['content'] = edits[aid]['content']
                art['edited'] = True
...
class Api:
    ...
    def get_materials(self, category=None):
        ...
        if category:
            ...
            rows = conn.execute(
                "SELECT * FROM materials ORDER BY category, name").fetchall()
        conn.close()
        out = []
        for r in rows:
            d = dict(r)
    ...
    def save_material(self, data):
        ...
        params = (
            ...
            float(data['price']), float(data.get('stock', 0)),
            float(data.get('stock_min', 0)), data.get('supplier_id'),
            meta, fav,
        )
        if data.get('id'):
            conn.execute(
                "UPDATE materials SET name=?, category=?, unit=?, price=?, stock=?, "
                "stock_min=?, supplier_id=?, meta=?, favorite=? WHERE id=?",
                params + (data['id'],))
        else:
            conn.execute(
                "INSERT INTO materials(name, category, unit, price, stock, stock_min, "
                "supplier_id, meta, favorite) VALUES(?,?,?,?,?,?,?,?,?)", params)
        conn.commit()
        conn.close()
        return {'ok': True}
    ...
    def get_projects(self):
        conn = get_conn()
        rows = conn.execute(
            "SELECT id, name, type, total, price, status, created_at FROM projects "
    ...
    def save_project(self, data):
        ...
        fields = (data['name'], data['type'], payload, float(data['total']),
                  float(data.get('price', 0)), data.get('status', 'draft'),
                  data.get('notes', ''))
        if data.get('id'):
            conn.execute(
                "UPDATE projects SET name=?, type=?, data=?, total=?, price=?, status=?, "
                "notes=? WHERE id=?", fields + (data['id'],))
            pid = data['id']
        else:
            cur = conn.execute(
                "INSERT INTO projects(name, type, data, total, price, status, notes) "
                "VALUES(?,?,?,?,?,?,?)", fields)
            pid = cur.lastrowid
        conn.commit()
        conn.close()
    ...
    def toggle_bookmark(self, article_id):
        conn = get_conn()
        exists = conn.execute(
            "SELECT 1 FROM bookmarks WHERE article_id=?", (article_id,)).fetchone()
        if exists:
    ...
    def save_article(self, article_id, title, content):
        conn = get_conn()
        conn.execute(
            "INSERT INTO library_edits(article_id, title, content, updated_at) "
            "VALUES(?,?,?,?) "
    ...
    def reset_article(self, article_id):
        conn = get_conn()
        conn.execute("DELETE FROM library_edits WHERE article_id=?", (article_id,))
        conn.commit()
        conn.close()
        return {'ok': True}
    ...
    def get_templates(self):
        conn = get_conn()
        rows = conn.execute("SELECT * FROM templates ORDER BY type, name").fetchall()
        conn.close()
        out = []
        for r in rows:
            d = dict(r)
    ...
    def save_template(self, data):
        ...
        if data.get('id'):
            conn.execute(
                "UPDATE templates SET name=?, type=?, description=?, data=? WHERE id=?",
                (data['name'], data['type'], data.get('description', ''),
                 payload, data['id']))
        else:
            conn.execute(
                "INSERT INTO templates(name, type, description, data) VALUES(?,?,?,?)",
    ...
    def save_supplier(self, data):
        ...
        if data.get('id'):
            conn.execute(
                "UPDATE suppliers SET name=?, contact=?, phone=?, email=?, website=?, "
                "notes=? WHERE id=?", fields + (data['id'],))
        else:
            conn.execute(
                "INSERT INTO suppliers(name, contact, phone, email, website, notes) "
                "VALUES(?,?,?,?,?,?)", fields)
        conn.commit()
        conn.close()
        return {'ok': True}
    ...
    def get_journal(self):
        conn = get_conn()
        rows = conn.execute(
            "SELECT * FROM journal ORDER BY created_at DESC").fetchall()
        conn.close()
        return [dict(r) for r in rows]
    ...
    def add_journal(self, data):
        conn = get_conn()
        conn.execute(
            "INSERT INTO journal(project_id, project_name, type, quantity, cost, "
            "revenue, note) VALUES(?,?,?,?,?,?,?)",
            (data.get('project_id'), data.get('project_name', ''),
             data.get('type', ''), int(data.get('quantity', 1)),
             float(data.get('cost', 0)), float(data.get('revenue', 0)),
             data.get('note', '')))
        conn.commit()
        conn.close()
        return {'ok': True}
    ...
    def get_low_stock(self):
        conn = get_conn()
        rows = conn.execute(
            "SELECT * FROM materials WHERE stock_min > 0 AND stock <= stock_min "
            "ORDER BY (stock - stock_min) ASC").fetchall()
        conn.close()
        out = []
        for r in rows:
            d = dict(r)
    ...
    def update_stock(self, mid, value):
        conn = get_conn()
        conn.execute("UPDATE materials SET stock=? WHERE id=?", (float(value), mid))
        conn.commit()
        conn.close()
        return {'ok': True}
    ...
    def export_backup(self):
        conn = get_conn()
        data = {}
        for table in ['materials', 'projects', 'settings', 'bookmarks',
                      'library_edits', 'templates', 'suppliers', 'journal']:
            try:
